...
弓兵ATK↑・DEF↑（戦士への打点改善・近接耐久強化）、魔導師HP↑・ATK↑（差別化維持）。
戦術AIは弓兵・魔導師が射程外からの攻撃位置を優先し、近接敵（range=1）を回避するよう改善済み。
...
- **企画・設計判断・全体俯瞰** は Web版Codex（このAGENTS.mdを更新）
- **実装・デバッグ・リファクタ** は Codex（このリポジトリで作業）
- 大きな設計変更が必要になったら Web版に相談 → AGENTS.md 更新 → Codex で実装
